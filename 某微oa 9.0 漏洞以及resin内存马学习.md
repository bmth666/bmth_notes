title: 某微oa 9.0 漏洞以及resin内存马学习
author: bmth
tags:
  - 内存马
  - 反序列化
top_img: 'https://img-blog.csdnimg.cn/134c804b89c0460bbb5451d303c9c220.png'
cover: 'https://img-blog.csdnimg.cn/134c804b89c0460bbb5451d303c9c220.png'
categories:
  - 代码审计
date: 2023-02-15 04:10:55
---
![](https://img-blog.csdnimg.cn/134c804b89c0460bbb5451d303c9c220.png)

最近看到了有关resin的内存马，觉得很有必要来研究，但又苦于没有源码，遂决定去fofa挖一下1day，然后分析分析漏洞
...
![](https://img-blog.csdnimg.cn/af3550b2739541d087be6b8071292c09.png)
...
成功写进我们的冰蝎马，连接拿下
![](https://img-blog.csdnimg.cn/9425bd1f3866435ca7b3a12e49ab7ed1.png)
...
```
成功连接冰蝎
![](https://img-blog.csdnimg.cn/706e77c9100042ca869759833f2741b6.png)
...
![](https://img-blog.csdnimg.cn/0e0dbc4edcaf49229a218e3fc38bf359.png)
...
![](https://img-blog.csdnimg.cn/06e1bdec169d41f79a7d9db06cb8e956.png)

访问`/weaver/className`，就会根据类名调用`CLASSPATH`下的servlet
我们可以看到在WEB-INF/lib目录下存在一个bsh-2[1].0b4.jar包
![](https://img-blog.csdnimg.cn/76b5698a8f404b1b85620c51a18f39c6.png)

我们导入包并看到类`bsh.servlet.BshServlet`，可以看到 doGet 方法从 getParameter 中接收到一些参数，然后整个Request请求会交给 evalScript 方法来进行处理
![](https://img-blog.csdnimg.cn/ecd30e935e0041e7a71e7b5ea6921060.png)

跟进 evalScript 方法，调用了bsh.Interpreter类的eval方法，执行了我们之前传入的字符串
![](https://img-blog.csdnimg.cn/3938854a015d4f6584492149b8c23244.png)

跟进 bsh.Interpreter类的eval方法，发现该类最终调用了bsh.commands/exec.bsh脚本
![](https://img-blog.csdnimg.cn/2271e71983004b6f8b85784fddcc1da4.png)
...
![](https://img-blog.csdnimg.cn/30ccd9402c324576a9c2b4ae0db90128.png)
...
```
![](https://img-blog.csdnimg.cn/53d02e4435994e378f57eff232f23ee7.png)
...
![](https://img-blog.csdnimg.cn/001ca798706541f99c94976fd17cc1e6.png)

跟进发现这个类继承了XFireServlet类，直接看到`org.codehaus.xfire.transport.http.XFireServlet`类
![](https://img-blog.csdnimg.cn/942cab9f979d45618c2f5915c0527acc.png)

servlet中post相当于触发doPost方法，看到调用了`XFireServletController#doService`方法，跟进
![](https://img-blog.csdnimg.cn/019aa07bae554db081785e7c0ed2602a.png)

getService方法获取服务名，也就是我们传入的WorkflowServiceXml，并且我们没有传wsdl，就会调用`XFireServletController#invoke`方法
![](https://img-blog.csdnimg.cn/3163e5f6590d418ab7e136d3c3891650.png)
...
![](https://img-blog.csdnimg.cn/1b577ef6bc164ec4a09ba5009970c71e.png)

通过xmlToObject解析我们传入的var1参数，跟进`XmlUtil#xmlToObject`
![](https://img-blog.csdnimg.cn/1ccb97314d5c4a428d71994f5d0b8a9c.png)
...
![](https://img-blog.csdnimg.cn/cace4cf120264f589fb2d4f7495ba65c.png)
...
![](https://img-blog.csdnimg.cn/067f337210154fedb62b7bd1dbcd960b.png)
...
%>
```
![](https://img-blog.csdnimg.cn/51ba7220542a42a095603a67d4cd9470.png)
...
![](https://img-blog.csdnimg.cn/fedc48c955e5418291b4dbf25cb9ac99.png)
...
```
![](https://img-blog.csdnimg.cn/d81768c297f743d187d8c01da1716d5d.png)
...
![](https://img-blog.csdnimg.cn/9d8ab416277f4c3e96552cd4884f77c6.png)

发现获取到的request对象为HttpRequest类型
![](https://img-blog.csdnimg.cn/f67dda99c1e3435da60cffd4975f6d48.png)
...
![](https://img-blog.csdnimg.cn/6050bdac2d084d02a2c577b7a0564a23.png)

```java
Object currentRequest = Thread.currentThread().getContextClassLoader().loadClass("com.caucho.server.dispatch.ServletInvocation").getMethod("getContextRequest").invoke(null);
WebApp webApp = (WebApp)currentRequest.getClass().getMethod("getWebApp").invoke(currentRequest);
```
![](https://img-blog.csdnimg.cn/45b6703922b8454598406f21dd53295d.png)
...
```
测试一下泛微oa，可以看到成功连接冰蝎
![](https://img-blog.csdnimg.cn/f12a1609031c43549392d2953f2de677.png)
